...
Blowfish_RC4plus::Blowfish_RC4plus(void){ } // Constructor

uint32_t Blowfish_RC4plus::F (uint32_t x)
{
	ushort a, b, c, d;
	uint32_t y;
...
void Blowfish_RC4plus::Blowfish_Encipher (uint32_t *L, uint32_t *R)
{
	uint32_t Xl, Xr;
...
/* Intialize Blowfish */
void Blowfish_RC4plus::Blowfish_Init (uchar *key, int keybytes)
{
...
uchar *Blowfish_RC4plus::Blowfish_key()
{
	srand(time(NULL));
...
/* Crack function */
void *Blowfish_RC4plus::crack_text (uchar *text, uint32_t *Left, uint32_t *Right, int len_text)
{
	int block_len;
	while (len_text)
	{
