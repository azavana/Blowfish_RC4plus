...
void Mutare::KSA (uchar *key, uchar *S)
{
	for (int i = 0; i < 256; i++)
		S[i] = i;
...
void Mutare::PRGA_RC4plus (uchar *S, uchar *plaintext, uchar *ciphertext)
{
	int i = 0;
	int j = 0;
...
void Mutare::RC4plus_Encrypt (uchar *plaintext, uchar *RC4plus_key, uchar *ciphertext)
{
	uchar S[256];
	KSA (RC4plus_key, S);
